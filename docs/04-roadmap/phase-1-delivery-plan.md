...
- 2026-07-03 已实现 RumorClaim 本地结构化 CLI MVP：`rumor add/list/show/dismiss/expire/promote`、持久化门槛、review queue、最小 source envelope、审计事件、`bundle --source rumor` 和 `bundle --source all --include-unverified` 的 lead-only 准入。
...
- `rumor add/list/show/dismiss/expire/promote` 最小 CLI 路径可运行。
