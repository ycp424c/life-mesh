...
Phase 1 typed persistence、Acceptance、`fact add/show/review/revoke`、source-triggered review 和 Bundle 准入已经实现。Fact 不存入 legacy `promoted_objects`。
...
`source_refs[]` 是支持事实的来源引用集合。Obsidian 等可编辑外部来源使用 Source Revision；Manual Input 不使用 SourceRevision，而引用 `input_id`、必要的 extraction id、`content_hash`、状态和审计事件。
...
Source Revision 或 Manual Input 变为 stale / missing / revoked / deleted 时，Canonical Fact 不立即删除。默认处理为：

1. 如果任一 required supporting source 失效，或不存在任何 current supporting source reference，设置 `validity=needs_review`，保留 `revocation_status=active`；optional support 失效但仍有其他 current support 时只记录 freshness/audit。
2. 如果来源被删除、移出索引范围或授权撤销，生成 Source Tombstone 或 Manual Input Tombstone，并触发依赖事实复核。
...
| 动作 | 结果 |
|---|---|
| `revalidate` | 绑定 current supporting source reference，恢复 `valid` |
