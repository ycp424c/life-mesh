...
| RumorClaim | 从可信度未知的文字片段、截图或图片中抽取出的未验证 claim；主资产是 claim 和 mentions，不默认保存原始物料。 |
| SourceEnvelope | RumorClaim 的最小来源外壳，用于审计和未来排查，不等同于 RawAsset。 |
...
- RumorClaim 默认不进入普通 Context Bundle；只有任务明确请求未验证线索时，才可作为 `lead` 进入，并必须标注未验证。
