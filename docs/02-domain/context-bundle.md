...
- **Canonical Fact 优先**：它已核实，但进入 Bundle 前必须检查 `validity`、`revocation_status` 和依赖 Source Revision 的新鲜度。只有 `valid + active + current-supported` 的事实能作为 `fact` slice。
...
- 依赖失效 Source Revision 且没有其他 current supporting revision 的 Canonical Fact 不立即删除，但必须标记为 `needs_review`，不能直接作为"已核实"使用。
...
- `needs_review`：进入 `freshness_report`，说明触发复核的 source revision 和原因。
- `revoked` / `invalid` / `superseded`：进入 `excluded_sources` 或审计视图，不进入可用上下文。
- Source Tombstone 影响的旧 revision 不能被新检索命中。
