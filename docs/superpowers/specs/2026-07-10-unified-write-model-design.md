# Unified Write Model Design

状态：approved in conversation，待 written-spec review
最后更新：2026-07-10
职责边界：定义 LifeMesh Phase 1 完整写模型的一次性交付与切换，包括统一 Candidate、Acceptance、Canonical Object、Provenance、Review、审计、数据库迁移、真实库备份恢复和 Bundle 准入。不定义外部日历/任务同步、自动 Rumor source adapter、远程服务或 dashboard 写回。
...
新增 `ADR-0010: Unified Write Model, Transactional Acceptance And Database Migration`，记录以下不可绕过的决策：
...
真实库迁移前基线只记录聚合计数，不输出个人内容：

- Manual Input 9：active 3、promoted 1、revoked 1、deleted 4。
- `promoted_objects` 1，类型 candidate。
- RumorClaim 87：candidate_created 6、dismissed 33、parked 8、reviewed_parked 40。
- `rumor_candidate_links` 6。
- Candidate 应回填 7：Manual Input 1 + RumorClaim 6。
- legacy payload 内有 4 个 source ref item，均可解析且与各自 primary Manual Input/Rumor source 相同；迁移时复用结构化 ref，不额外创建 link 或 opaque ref。
- 规范化 source reference 预期 96：Manual Input 9 + RumorClaim 87。
- source tombstone 预期 38：revoked/deleted Manual Input 5 + dismissed RumorClaim 33。
- 正式 Fact/Memory/Task/Event 当前均为 0。
- 初始 review item 0。
- 有 1 条 deleted input 的 audit-only promotion；只迁 audit，禁止复活成第 8 条 Candidate。
...
- `promoted_objects(candidate)` → Candidate + Manual Input `derived_from` primary source link；status=pending、confirmation_required=1、sensitivity=max(source)。
...
9. 回填 7 条 Candidate 和 7 条 primary source link；legacy payload 内 4 个同源 ref 规范化后与 primary link 去重。
10. 回填 9 条 Manual Input 和 87 条 RumorClaim source reference；Candidate link 引用这些统一 ref。
...
- 重跑 migration 后 Candidate 仍为 7，不得变成 14。
...
1. 停止 LifeMesh 连接。
2. 将失败库保留为 forensic 副本。
3. 校验原备份 SHA-256 和 integrity。
4. 恢复到同目录临时文件，设置 `0600`，fsync 文件和父目录。
5. 使用 `os.replace` 原子替换数据库。
6. 清理与失败库关联的 WAL/SHM/journal companion。
7. 复查 integrity、旧表聚合计数和 CLI 只读 smoke test。
8. 保留备份和 forensic 库，直到人工确认完成。
...
- candidate source links=7
- source references=96
- source tombstones=38
- 正式对象初始计数=0
- review items=0
...
本次是重大架构决策，必须新增 ADR-0010，并同步：

- `docs/02-domain/`：Candidate、Canonical Fact/Object、Memory、Provenance、Manual Input、RumorClaim、Context Bundle、Data Map。
- `docs/03-architecture/`：新增 `write-model-and-migrations.md`，并更新 overview、data layers、system map、CLI、Agent access、security/audit。
...
5. 7 条 legacy Candidate handoff 可在统一 inbox 查询。
6. 真实库旧业务计数和向量检索没有回归。
